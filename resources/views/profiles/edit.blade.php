...
<form action="{{ route('profiles.edit', ['id' => $profile['id']]) }}" method="post" enctype="multipart/form-data">
 @csrf
...
<label>User Name:</label>
   <input type="text" name="name" value="<?=$profile['name']?>" placeholder="enter your name" class="box" required>
   ...
   <input type="date" name="datebirth" id="DateOfBirth" value="<?=$profile['datebirth']?>" placeholder="enter datebirth" class="box" required>
   ...
   <input type="text" name="age" id="Age" value="<?=$profile['age']?>" placeholder="calculate extact age" class="box" required>
   @error('age')
                             <span class="invalid-feedback" role="alert">
                                 <strong>{{ $message }}</strong>
                             </span>
 ...
 <button style="color: #000; float: inline-start; height: 35px; background-color: #eee; " id="selectedRole" name="role" id="role" value="{{ old('role', $profile['role'] ?? '') }}" class="btn btn-default dropdown-toggle" type="button" data-toggle="dropdown">Role
 <?= $profile['role'] ?? 'Role' ?>
 ...
 <li class="dropdown-submenu">
     ...
     <ul class="dropdown-menu">
                 <li><a tabindex="-1" href="#"  value="Batsman" <?= ($profile['role'] == 'Batsman') ? 'selected' : '' ?>>Batsman</a></li>
                 <li><a tabindex="-1" href="#" onclick="setDropdownValue('Opener', '<?= $profile['role'] ?>')">Opener</a></li>
                 <li><a tabindex="-1" href="#" onclick="setDropdownValue('Middle Order', '<?= $profile['role'] ?>')">Middle Order</a></li>
                 <li><a tabindex="-1" href="#" onclick="setDropdownValue('Tail', '<?= $profile['role'] ?>')">Tail</a></li>
             </ul>
   </li>
   <li class="dropdown-submenu">
     <a class="test" tabindex="-1" href="#">Bowler <span class="caret"></span></a>
     <ul class="dropdown-menu">
     <li><a tabindex="-1" href="#" onclick="setDropdownValue('Bowler', '<?= $profile['role'] ?>')">Bowler</a></li>
     <li><a tabindex="-1" href="#" onclick="setDropdownValue('Fast', '<?= $profile['role'] ?>')">Fast</a></li>
     <li><a tabindex="-1" href="#" onclick="setDropdownValue('Middle Fast', '<?= $profile['role'] ?>')">Middle Fast</a></li>
     <!-- <li><a tabindex="-1" href="#" onclick="setDropdownValue('slow', '<?= $profile['role'] ?>')">slow</a></li> -->
     <li><a tabindex="-1" href="#" onclick="setDropdownValue('Off Spin', '<?= $profile['role'] ?>')">Off Spin</a></li>
     <li><a tabindex="-1" href="#" onclick="setDropdownValue('Left-Arm Orthodox Spin', '<?= $profile['role'] ?>')">Left-Arm Orthodox Spin</a></li>
     <li><a tabindex="-1" href="#" onclick="setDropdownValue('Leg Spin', '<?= $profile['role'] ?>')">Leg Spin</a></li>
     <li><a tabindex="-1" href="#" onclick="setDropdownValue('Left-Arm UnOrthodox Spin', '<?= $profile['role'] ?>')">Left-Arm UnOrthodox Spin</a></li>
   ...
   <li class="dropdown-submenu">
     ...
     <ul class="dropdown-menu">
     <li><a tabindex="-1" href="#" onclick="setDropdownValue('All Rounder', '<?= $profile['role'] ?>')">All Rounder</a></li>

     <li><a tabindex="-1" href="#" onclick="setDropdownValue('Batting All Rounder', '<?= $profile['role'] ?>')">Batting All Rounder</a></li>
     <li><a tabindex="-1" href="#" onclick="setDropdownValue('Bowling All Rounder', '<?= $profile['role'] ?>')">Bowling All Rounder</a></li>

     </ul>
   </li>
   <li class="dropdown-submenu">
   <li><a tabindex="-1" href="#"  value="Wicket Keeper" <?= ($profile['role'] == 'Wicket Keeper') ? 'selected' : '' ?>>Wicket Keeper</a></li>
...
<select id="country-state" name="state" class="form-select"> 
 <option value="">Select state</option>
 <option value="Andaman and Nicobar Islands" <?= ($profile['state'] == 'Andaman and Nicobar Islands') ? 'selected' : '' ?>>Andaman and Nicobar Islands</option>
 <option value="Andhra Pradesh" <?= ($profile['state'] == 'Andhra Pradesh') ? 'selected' : '' ?>>Andhra Pradesh</option>
 <option value="Arunachal Pradesh" <?= ($profile['state'] == 'Arunachal Pradesh') ? 'selected' : '' ?>>Arunachal Pradesh</option>
 ...
 <option value="Dadra and Nagar Haveli" <?= ($profile['state'] == 'Jammu and Kashmir') ? 'selected' : '' ?>>Dadra and Nagar Haveli</option>
 <option value="Daman and Diu" <?= ($profile['state'] == 'Jammu and Kashmir') ? 'selected' : '' ?>>Daman and Diu</option>
 ...
 <option value="Himachal Pradesh" <?= ($profile['state'] == 'Himachal Pradesh') ? 'selected' : '' ?>>Himachal Pradesh</option>
 <option value="Jammu and Kashmir" <?= ($profile['state'] == 'Jammu and Kashmir') ? 'selected' : '' ?>>Jammu and Kashmir</option>
...
<label>Select City:</label>
 ...
 <option value="Ahmadnagar" <?= ($profile['city'] == 'Ahmadnagar') ? 'selected' : '' ?>>Ahmadnagar</option>
...
<label>User Email:</label>
   <input type="email" name="email" value="<?=$profile['email']?>" placeholder="enter email"  class="box" required readonly>
            ...
            <input type="radio" id="bowling_left" name="bowling_orientation" value="Left Hand" <?= ($profile['bowling_orientation'] == 'Left Hand') ? 'checked' : '' ?>>
            <label for="bowling_left">Left Hand</label>
            <input type="radio" id="bowling_right" name="bowling_orientation" value="Right Hand" <?= ($profile['bowling_orientation'] == 'Right Hand') ? 'checked' : '' ?>>
            <label for="bowling_right">Right Hand</label><br>
            </div>

            <div class="raddddio" name="raddddio">
            <h4>Batting Orientation</h4>
            <input type="radio" id="batting_left" name="batting_orientation" value="Left Hand" <?= ($profile['batting_orientation'] == 'Left Hand') ? 'checked' : '' ?>>
            <label for="batting_left">Left Hand</label>
            <input type="radio" id="batting_right" name="batting_orientation" value="Right Hand" <?= ($profile['batting_orientation'] == 'Right Hand') ? 'checked' : '' ?>>
        ...
        @if($profile['file'])
            ...
            @endif
   <input type="submit" name="submit" value="Update Now" class="btn" style="background-color: blue; color:#fff">
</form>
...
<!-- <label class="m-2">Cover Image</label>
                 ...
                 <label class="m-2">Photos</label>
                    ...
                    <div class="image-box">
                    <img src="/images/{{ $photo->photo }}" class="img-responsive" style="max-height: 100px; max-width: 100px;" alt="" srcset="">
                    </div>
                    @endforeach
                    @endif
                 </div>
